...
- The target: a notebook `id` (or `alias`) + a deck folder under `inbox/`.
  No notebook yet? `node tools/new-notebook.mjs "<Subject>" <unitNo> "<Title>"` (or bun) — it creates
...
runs `node tools/validate-notebook.mjs <id>` (or bun); sets registry `status: "ready"` only on pass;
...
### 7. Doubt skill
On a green validate: `node tools/gen-doubt-skill.mjs <id>` (or bun). Tick PROGRESS.
